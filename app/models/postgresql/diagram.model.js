...
export class DiagramModel {
    ...
    getDiagramsByUser = async({ userId }) => {
    ...
    getDiagramInfo = async({ userId, diagramId }) => {
        const { rows, rowCount } = await pool.query(
    ...
    softDeleteDiagram = async({ userId, diagramId }) => {
        const { rowCount } = await pool.query(`
    ...
    restoreDiagram = async({ userId, diagramId }) => {
    ...
    deleteDiagram = async({ userId, diagramId }) => {
        const { rows, rowCount } = await pool.query(
            `SELECT status 
            FROM user_diagrams 
            WHERE template_id = $1 AND user_id = $2`,
    ...
    createDiagramUser = async({ userId, input, previewImage }) => {
        const { rows, rowCount } = await pool.query(
    ...
    updateDiagramUser = async({ userId, diagramId, input, preview_image }) => {
        const { rowCount: exists } = await pool.query(
            `SELECT template_id FROM user_diagrams 
            WHERE template_id = $1 AND user_id = $2 AND status = $3`,
            [ 
    ...
    setFavoriteStatus = async({ userId, diagramId, input }) => {
        const { rowCount: exists } = await pool.query(
            `SELECT template_id FROM user_diagrams
            WHERE template_id = $1 AND user_id = $2 AND status = $3`,
            [
    ...
    getUserFavoriteDiagrams = async({ userId }) => {
        const { rows, rowCount } = await pool.query(
            `SELECT template_id, name, description, template_data, created_at, is_favorite, preview_image
            FROM user_diagrams
    ...
    getUserDiagramsInTrash = async({ userId }) => {
        const { rows, rowCount } = await pool.query(
